...
export class CoverLetterSettingTab extends PluginSettingTab {
    ...
    private buildModelCombo(
        ...
        setting.addText((t) => {
            ...
            // Store for targeted updates to avoid full page redraws (jumping)
            this.modelDatalists.set(datalistId, datalist);
            this.modelFetchers.set(datalistId, getModels);
    ...
    async updateModelLists(): Promise<void> {
        for (const [id, datalist] of this.modelDatalists) {
            const fetcher = this.modelFetchers.get(id);
            if (!fetcher) continue;
